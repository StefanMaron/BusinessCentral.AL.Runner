...
internal static partial class ProgramSupport
{
    ...
    internal static void PrintGuide(TextWriter w)
    {
        ...
        w.WriteLine();
        w.WriteLine("  HOW THE WINNER IS PICKED — and how NOT to check it. Among the copies of a");
        w.WriteLine("  package at or above the declared minimum, across ALL scanned directories, a");
        w.WriteLine("  copy with an R2R DLL wins first, then one the runner can compile (AL source,");
        ...
        w.WriteLine("  meets the minimum — and then the run prints a NO IMPLEMENTATION entry listing");
        w.WriteLine("  every other copy it found and the directories it searched.");
        w.WriteLine();
        w.WriteLine("  CALIBRATION: --verbose also prints a [dep] note: line when a symbols-only copy");
        w.WriteLine("  won because every runnable copy is below the minimum. That is EVIDENCE TO WEIGH,");
        w.WriteLine("  not a verdict: treat it as the first place to look when execution fails, not");
        w.WriteLine("  as proof of the cause.");
        w.WriteLine();
    ...
    internal static void PrintHelp(TextWriter w)
    {
        ...
        w.WriteLine("                          retry runs only work no attempt reached. Default 5; 0");
        w.WriteLine("                          disables. A hung test abandons the");
        ...
        w.WriteLine("                          depends on how this binary was installed: a");
        w.WriteLine("                          single-build install (built from source)");
        ...
        w.WriteLine("                          tests. Useful for pre-warming caches. Takes no");
        w.WriteLine("                          --bc-version: it selects the BC artifact version equal to");
        w.WriteLine("                          the .app's own manifest Version, and falls back to the");
        w.WriteLine("                          latest version in the artifacts cache when no such");
