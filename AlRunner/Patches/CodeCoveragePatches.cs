...
public static class CodeCoveragePatches
{
    ...
    {
        // A missing type or property is a BC shape change (ShapeGap); a null VALUE is the
        // runner having no resolver (out of scope). Keep the two apart (#3663).
        var navGlobal = _getMetaApplicationObject!.DeclaringType!.Assembly
            .GetType("Microsoft.Dynamics.Nav.Runtime.NavGlobal")
            ?? throw ShapeGap("type Microsoft.Dynamics.Nav.Runtime.NavGlobal not found");
        var resolverProperty = navGlobal.GetProperty("NavAppGroupResolver",
            BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)
            ?? throw ShapeGap("property NavGlobal.NavAppGroupResolver not found");
    ...
    private static Delegate MakeFunc(Type funcType, string helperName)
    {
        var helper = typeof(CodeCoveragePatches).GetMethod(helperName, BindingFlags.Public | BindingFlags.Static)
            ?? throw new InvalidOperationException($"CodeCoveragePatches.{helperName} not found");
    ...
    private static InvalidOperationException ShapeGap(string what)
        => new($"CodeCoveragePatches: {what}. BC's ALCodeEnvironment/NCLMetadata shape has changed; the "
