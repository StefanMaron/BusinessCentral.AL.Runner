...
//                                          values ALSO stamped onto this app's AllObj rows
//
//   Deliberately NOT filled: Installed and Tenant Visible are FlowFields on this table
//   (verified against this artifact's own Field metadata), so a write to either is silently
//   discarded — it would have looked like an answer and been none.
//
...
public static partial class RecordPatches
{
    ...
    private static void SeedPublishedApplicationRows(bool includeBundle, string what)
    {
        ...
        int seeded = 0, alreadyPresent = 0;
        var failed = new List<string>();
        foreach (var m in modules)
        {
            try
            {
                InsertPublishedApplicationRow(meta, source, m);
                seeded++;
            }
            catch (Exception ex)
            {
                ...
                {
                    alreadyPresent++;   // already present — not a failure.
                    continue;
                }
                failed.Add($"{m.Publisher}_{m.Name} v{m.Version}: {inner.GetType().Name}: {inner.Message}");
            }
        }
        ...
        if (failed.Count > 0)
            throw new InvalidOperationException(
                ...
                + $"{what} row(s) into table {PublishedApplicationSystemTableId}; refusing to continue "
        ...
        (Guid AppId, string Name, string Publisher, string Version) module)
    {
        ...
        if (!fieldByName.ContainsKey("ID"))
            throw new InvalidOperationException(
                $"Published Application metatable ({PublishedApplicationSystemTableId}) has no "
                + $"\"ID\" field [fields={string.Join("/", fieldByName.Values.Select(f => $"{f.FieldNo}:{f.FieldName}"))}] "
        ...
        // BC filters Tenant ID with '%1|%2' against ('', TenantInformation.GetTenantId()), so
        // the empty string matches without the runner inventing a tenant id it does not have.
        ...
        var dataAccess = perTable.GetOrAdd(PublishedApplicationSystemTableId,
            _ => _mCreateTempDataAccess!.Invoke(source, new object[] { meta })!);
        var provider = GetDataProvider(dataAccess)
            ?? throw new InvalidOperationException(
                $"Published Application ({PublishedApplicationSystemTableId}) data access exposes "
                + "no in-memory DataProvider");
