...
internal static partial class BcAppSymbolCache
{
    ...
    // a Dictionary<,> value or key is descended into like any other. A runner-owned generic is
    // walked regardless, through IsOwnType; a BCL generic only if named there. Tuple<,>,
    // ValueTuple, ConcurrentDictionary<,> and SortedDictionary<,> are NOT (measured) -- add one
    // to that set before putting it in a payload.
    //
    ...
    // every leg provisions a fresh cache.
    //
    // Trap: re-read this constant on origin/main immediately before pushing a bump. A rebase
    // merges the text and cannot tell you the number is taken; v39 collided with #3791 that way.
    // Two branches sharing an integer fail as described at PayloadShape.
    //
    ...
    internal sealed record PageExtensionSymbol(
        ...
        // Trailing + optional so a v30 payload still deserialises; guarded by the v31
    ...
    /// cache miss" CacheVersion's own comment warns about, reproduced across branches
