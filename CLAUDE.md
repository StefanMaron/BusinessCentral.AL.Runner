...
- Work ONE issue yourself, in the main session, all the way to a merged PR (claim, RED, fix, mutation-check,
  attribute the failures that are not yours, external review through `pi`, PR, GitHub's automatic review) →
  skill `driving-an-issue-to-merge`
