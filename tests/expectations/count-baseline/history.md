...
## runner-extras `date-virtual-table-window` 15 -> 7 (#3506)
...
was rewritten to seven arms that each assert a LOWER BOUND past a number the runner used to
impose (a count above the old cap, a first row before the 1900 edge, a last row past the 2099
edge, a keyed Get eight centuries outside any store), plus one exact in-window control. Measured
7P/0F/0E on the rewritten bundle, not computed from the diff.
