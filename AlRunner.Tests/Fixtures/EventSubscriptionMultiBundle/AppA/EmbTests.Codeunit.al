...
//   `ForeignSubscriptionIsAbsentA` filters on the OTHER bundle's subscriber codeunit id,
...
//   `AllObjDoesNotListTheForeignCodeunitA` asks a DIFFERENT virtual table the same
...
codeunit 70784 "EMB Tests A"
{
    Subtype = Test;
    TestPermissions = Disabled;

    var
        Assert: Codeunit "EMB Assert A";

    [Test]
    procedure OwnSubscriptionIsPresentA()
    ...
    begin
        ...
        EventSubscription.SetRange("Subscriber Codeunit ID", Codeunit::"EMB Subscriber A");
        Assert.AreEqual(
    ...
    [Test]
    procedure ForeignSubscriptionIsAbsentA()
    // The arm this issue is about. 70803 is the OTHER bundle's subscriber codeunit,
    // an object this bundle never declares and cannot reference by name.
    var
        EventSubscription: Record "Event Subscription";
    begin
        EventSubscription.Reset();
        EventSubscription.SetRange("Subscriber Codeunit ID", 70803);
        Assert.AreEqual(
    ...
    [Test]
    procedure AllObjDoesNotListTheForeignCodeunitA()
    ...
    begin
        ...
        AllObj.SetRange("Object ID", 70803);
        Assert.AreEqual(
            0, AllObj.Count(),
