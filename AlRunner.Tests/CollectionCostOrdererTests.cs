...
public sealed class CollectionCostOrdererTests
{
    ...
    public void Order_HandsBackTheMeasuredHeaviestCollectionsFirst()
    {
        ...
        Assert.Equal("CacheKeyDependencyClosureTests", ordered[0]);
        Assert.Equal("ServerCancelTests", ordered[1]);
    ...
    public void Order_PacksFourThreadsWithinTenPercentOfTheWorkDividedByFour()
    {
        ...
        Assert.Equal(measured.Keys.OrderBy(k => k, StringComparer.Ordinal),
                     ObservedDispatchOrder.OrderBy(k => k, StringComparer.Ordinal));
