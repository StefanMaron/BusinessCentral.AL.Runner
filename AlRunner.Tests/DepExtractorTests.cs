...
public class DepExtractorTests
{
    ...
    /// <summary>
    /// Regression for issue #1521 sub-gap: when a consumer file declares
    /// <c>namespace Microsoft.Foo;</c> and references an unresolved object
    /// unqualified (e.g. <c>"Phantom NS Enum"</c>), BC name resolution searches
    /// the file's namespace + its `using` directives but NOT the global root.
    /// A stub emitted at the root is therefore invisible to the consumer.
    /// We must emit a copy of the stub inside <c>namespace Microsoft.Foo;</c>.
    /// </summary>
    [Fact]
    public void ExtractDeps_AutoStub_EmitsStubInConsumerNamespace_ForUnresolvedEnumReference()
    {
        var depRoot = Path.Combine(Path.GetTempPath(), "al-dep-nsstub-" + Guid.NewGuid().ToString("N")[..8]);
        var outDir  = Path.Combine(Path.GetTempPath(), "al-out-nsstub-" + Guid.NewGuid().ToString("N")[..8]);
        var extDir  = Path.Combine(Path.GetTempPath(), "al-ext-nsstub-" + Guid.NewGuid().ToString("N")[..8]);
        try
        {
            Directory.CreateDirectory(Path.Combine(depRoot, "AppA"));
            Directory.CreateDirectory(extDir);
            // Consumer in `namespace Microsoft.Foo;` references unqualified enum.
            File.WriteAllText(Path.Combine(depRoot, "AppA", "AppACodeunit.al"), """
                namespace Microsoft.Foo;
                codeunit 60500 AppACodeunit
                {
                    procedure DoSomething()
                    var
                        e: Enum "Phantom NS Enum";
                    begin
                    end;
                }
                """);
            File.WriteAllText(Path.Combine(extDir, "Consumer.al"), """
                codeunit 60599 Consumer
                {
                    procedure Run()
                    begin
                        Codeunit.Run(Codeunit::AppACodeunit);
            ...
            var nsStub = allFiles.FirstOrDefault(f =>
            {
                if (!f.Contains("__GeneratedStubs__")) return false;
                var c = File.ReadAllText(f);
                return c.Contains("namespace Microsoft.Foo;")
                    && c.Contains("\"Phantom NS Enum\"");
            });
            Assert.NotNull(nsStub);
        }
        finally
    ...
    /// <summary>
    /// Negative variant: when the consumer file has no <c>namespace</c>
    /// declaration, the stub stays at the global root (no namespace prefix).
    /// </summary>
    [Fact]
    public void ExtractDeps_AutoStub_EmitsRootStub_ForConsumerWithoutNamespace()
    {
        var depRoot = Path.Combine(Path.GetTempPath(), "al-dep-rootstub-" + Guid.NewGuid().ToString("N")[..8]);
        var outDir  = Path.Combine(Path.GetTempPath(), "al-out-rootstub-" + Guid.NewGuid().ToString("N")[..8]);
        var extDir  = Path.Combine(Path.GetTempPath(), "al-ext-rootstub-" + Guid.NewGuid().ToString("N")[..8]);
        try
        {
            Directory.CreateDirectory(Path.Combine(depRoot, "AppA"));
            Directory.CreateDirectory(extDir);
            File.WriteAllText(Path.Combine(depRoot, "AppA", "AppACodeunit.al"), """
                codeunit 60510 AppACodeunit
                {
                    procedure DoSomething()
                    var
                        e: Enum "Phantom Root Enum";
                    begin
                    end;
                }
                """);
            File.WriteAllText(Path.Combine(extDir, "Consumer.al"), """
                codeunit 60598 Consumer
                {
                    procedure Run()
                    begin
                        Codeunit.Run(Codeunit::AppACodeunit);
            ...
            var stubs = Directory.GetFiles(outDir, "*.al", SearchOption.AllDirectories)
                .Where(f => f.Contains("__GeneratedStubs__")
                            && File.ReadAllText(f).Contains("\"Phantom Root Enum\""))
                .ToList();
            Assert.NotEmpty(stubs);
            // None of the emitted copies for this object should carry a `namespace ...;` prefix.
            foreach (var s in stubs)
                Assert.DoesNotContain("namespace ", File.ReadAllText(s));
        }
        finally
        {
            foreach (var d in new[] { depRoot, outDir, extDir })
