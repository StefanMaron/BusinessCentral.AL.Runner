...
if [ -n "$stray_number" ]; then
  ...
  echo "::error::This PR's $stray_source contains a closing keyword (one of Close/Closes/Closed/Fix/Fixes/Fixed/Resolve/Resolves/Resolved, case-insensitive) next to issue number $stray_number, somewhere other than its own standalone 'Closes #N' trailer line -- in a sentence, a table row, a list item or a code span; GitHub's parser does not see markdown, so backticks and fences do not protect the text. This repo squash-merges the PR title + body into the merge commit message, and that parser fires on the pattern ANYWHERE in the message -- it does not understand negation, tense or surrounding qualification. Whatever the sentence says, this will close issue $stray_number on merge (this happened for real: PR #2127's body said a sentence did NOT close an issue, and merge commit fe789a13 closed it anyway; and a true PAST-TENSE mention of an already-closed issue fires identically, which is how two queue-scan paragraphs tripped this in four hours -- #4294). READ THE NUMBER ABOVE: it is the one that will close, and in a list or a table row that is not always the one your sentence is about, because the separator matches a punctuation mark and hands the keyword to the NEXT number on the line -- '#111 - closed, #222 - open' closes #222, not #111. If issue $stray_number is not one of this PR's declared canonical targets, put a WORD between the keyword and the number, or drop the keyword: 'closed via #$stray_number', 'closed by #$stray_number', 'fixed in #$stray_number', 'see #$stray_number'. The separator cannot span a word, so each of those still creates the cross-reference on merge without the closing behavior. If it IS meant to close, add its own standalone 'Closes #$stray_number' line." >&2
  exit 1
fi
