...
| `NavForm.GetAutoFormatStringAsync` | BC's body dispatches the AutoFormat extension event and answers `""` only when nothing handled it; the runner's Cecil rewrite (`NclCecilRewrite.Forms.cs`) answers `""` on every call, so a page field with `AutoFormatType <> 0` formats with the default rather than BC's currency-aware string — **a silent default, not a `not-yet-implemented` throw**, contrary to the paragraph above this table. Faithful for `AutoFormatType = 0`. | [#3406](https://github.com/StefanMaron/BusinessCentral.AL.Runner/issues/3406) |
