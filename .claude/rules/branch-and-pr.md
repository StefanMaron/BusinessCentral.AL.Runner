...
- **Assign the issue to `@me` when you claim it** (`gh issue edit <N> --add-assignee @me`, or `mcp__github__issue_write` with `method: update`), and skip any issue or PR assigned to a user other than `@me`: the assignee is the boundary between agent-owned and human-owned work on a public repo, and only the repo owner waives it. Between agents it decides nothing, because every loop pushes under one account — read the open PRs instead, and never release a claim by removing an assignee (`check-open-prs-before-claiming.md`).
