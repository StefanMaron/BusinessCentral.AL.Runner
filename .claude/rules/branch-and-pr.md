...
- **A branch named `agent/<id>/issue-<N>` must say what the PR does about N** — either the body's `Closes #N`, or a standalone `Part of #N` line when the PR lands part of the issue and N stays open. `pr-gate.yml` enforces it and `.github/workflows/issue-label-hygiene.yml` reads the `Part of` line on merge to put N back on the ready queue (#3678). Trap: `Part of #N` for the branch's own issue is itself a declaration, so a partial landing never needs the `No linked issue:` escape hatch. **Second trap: "standalone" means the line ENDS after the issue number.** `check_closing_reference.sh`'s `PART_OF_LINE_RE` anchors with `$`, so `Part of #1883 - the NavDataTransfer cluster` is reported as the declaration being **absent**, not malformed - and the error text then tells you to add a line the body already appears to have. Put the prose on the next line. Verified both ways by running the gate against both spellings (#3927).
