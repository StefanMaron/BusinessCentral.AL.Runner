...
- Expectation-manifest drift is the orchestrator's merge pass to fix, not yours: on a known-gap entry left behind after its issue closed, or a red `main` from manifest drift, comment the entry and what it now does on the issue that entry's `Issue` field names, and carry on with your own task. Done when that comment exists.
