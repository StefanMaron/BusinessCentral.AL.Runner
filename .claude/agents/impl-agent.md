...
4. **The repository's own guards, if you added or moved a file.** They are fast, they do not
   ...
   dotnet test AlRunner.Tests --filter "FullyQualifiedName~GuardTests"
   ```

   `.github/scripts/test_*` holds further guards, run by `pr-gate.yml` rather than by this loop,
   and that set is MIXED — `.py` and `.sh` — so `for s in .github/scripts/test_*; do bash "$s"; done`
   reports every `.py` as a failure that is not there. It has cost two reviewers a round each
   (#4509). Dispatch on the extension:
   ...
   `ls` counts either set; no count is written here, because every guard-adding PR would edit
   it the same way and two such PRs merge cleanly into a red `main` (#4248, #4235). Four fired on a new
