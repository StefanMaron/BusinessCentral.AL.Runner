...
7. **Read the review verdict before arming.** The newest review comment on the PR must end with
   a `Verdict: MERGE` line whose head equals the PR's current head (`gh pr view <N> --json
   headRefOid`). A FIX-FIRST or HOLD, a missing verdict line, or a head that has moved since:
   do not arm and do not merge — say which and hand the PR back. The rest of the arming
   conditions are one list, in the `orchestrating-a-session` skill under "A reviewer that
   approves a PR arms auto-merge".
