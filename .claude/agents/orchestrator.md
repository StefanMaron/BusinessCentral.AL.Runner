...
tools: Bash, Read, Grep, ToolSearch, mcp__github__get_me, mcp__github__list_issues, mcp__github__issue_read, mcp__github__issue_write, mcp__github__list_pull_requests, mcp__github__pull_request_read, mcp__github__merge_pull_request, mcp__github__update_pull_request, mcp__github__add_issue_comment, mcp__github__get_job_logs
model: sonnet
---
...
**GitHub access:** `gh` does not exist in web/remote sessions. Detect once at the start of the pass and use `gh` or the `mcp__github__*` tools accordingly — see `.claude/rules/github-access.md` for the operation→tool map. The `gh` commands below are the local-CLI spelling; each has an MCP equivalent in that table. When `gh` is available, pass `--repo StefanMaron/BusinessCentral.AL.Runner` on every command.
...
5. **`tests/al-language/` modified** → the corpus submodule is read-only here. The only legitimate change to that path is a pin bump, which is its own PR. If a PR edits corpus files, block with:
   > `tests/al-language/` is a read-only submodule pinned at the upstream corpus — a failing corpus test is a runner gap, not a corpus bug. Please revert these files and land the fix in the runner (or declare it in `tests/expectations/`). If the corpus itself is wrong, open a PR against `StefanMaron/BusinessCentral.AL.Language.Tests` and bump the pin separately. See `.claude/rules/al-language-submodule.md`.

   There is **no `coverage.yaml` check** — v1's hand-curated coverage map was retired at the v1→v2 cutover and archived to `docs/archive/coverage.yaml`. In v2 the coverage record *is* the `tests/al-language/` corpus plus `tests/runner-extras/`; do not ask a PR to update a coverage file.
...
7. Sanity check passed (step 1) + CI green + no CHANGELOG + no corpus edits + no forbidden SA implementation:
...
- No merge if the PR edits `tests/al-language/` outside a dedicated pin-bump PR.
