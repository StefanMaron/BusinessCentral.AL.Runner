...
- **testpage:** request-page Visible/Editable bound to a report global on a precompiled report
...
- **report:** apply the transaction-world guard to RunRequestPage, Execute and Print
...
- **runner-extras:** pin [TryFunction] vs plain no-exit returns through the precompiled path
- **testpage:** read a NavDateTime round trip through GetClientLocalValue, not .Value
...
- **spawn:** derive the 180s cohort's timeout messages from the cap applied
- **testpage:** check the skeleton-session precondition instead of asserting it
